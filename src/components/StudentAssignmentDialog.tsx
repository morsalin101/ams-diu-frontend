            ...
            <div className="space-y-4 sm:space-y-5 flex flex-col h-full min-h-[320px] xl:min-h-0 xl:overflow-y-auto pr-1">
              ...
              <div className="flex-1 space-y-5">
                ...
                <div className="space-y-2 sm:space-y-3">
                  ...
                  >
                    <SelectTrigger
                      className="h-11 sm:h-12 !py-1 text-xs sm:text-sm items-center"
                      aria-label="Schedule"
                    >
                      <SelectValue
                        placeholder="Select a schedule"
                        className="text-center !whitespace-normal !overflow-visible text-clip leading-tight"
                      />
                    </SelectTrigger>
                    <SelectContent className="!min-w-[var(--radix-select-trigger-width)]">
                      {filteredSchedules.length === 0 ? (
                        ...
                        filteredSchedules.map(schedule => (
                          <SelectItem key={schedule.id} value={schedule.id.toString()} className="!py-2 !px-4 text-xs sm:text-sm">
                            <div className="flex flex-col items-center text-center min-w-0 gap-0">
                              <span className="font-semibold whitespace-normal break-words text-xs sm:text-sm text-center leading-tight text-gray-800">
                                {new Date(schedule.start_time).toLocaleTimeString()} - {new Date(schedule.end_time).toLocaleTimeString()}
                              </span>
                              <span className="text-gray-500 whitespace-normal break-words text-[10px] sm:text-xs text-center leading-tight">
